...
# run example:
# ./scripts/runCloudlab.sh 120 150 6 12 95 0 25 true true true 1
if [ "$#" -lt 11 ]; then 
    echo  "Usage: $0 <duration:sec> <#clis> <#servers> <#nodes> <locality> <#msgs> <#clispernode> <payload> <thinktime> <localm> <tree>"
    exit 0; 
fi
...
clispernode=$clispernode  payload=$payload thinktime=$thinktime" tree=$tree >> $basedir/logs/execution.log;
echo "------------------------------------------------------------------------------------------------" >> $basedir/logs/execution.log;
...
do
    ...
    ./scripts/sshcli.sh $node $basedir $clients $ID $duration $locality $warehouse $msgs $log $clispernode $payload $thinktime $localm $tree  # >> $basedir/logs/execution.log;
    sleep 1;
    ID=$(($ID+$clispernode));
    lastnode=$node;
done < <( awk '!/^ *#/ && NF'  "$clifile");
